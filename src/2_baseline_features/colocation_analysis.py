...
def readCheckins():
    ...
    dtype = [("users_ID", int), ("time", int), ("latitude", "S6"), ("longitude", "S7")]
    vert1 = lambda x: time.mktime(time.strptime(x.decode(), '%Y-%m-%dT%H:%M:%SZ'))
    vert2 = lambda x: x[:6]
    vert3 = lambda x: x[:7]
...
def intersection(set01, set02):
    ...
    for e1 in set01:
        for e2 in set02:
            if (e1[1] == e2[1]) and abs(e1[0] - e2[0]) <= 3600:
